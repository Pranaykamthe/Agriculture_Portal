                        ...
                        <div class="card-body">
                            <h4>
                                ...
                                if (isset($_POST['Crop_Predict'])) {
                                    ...
                                    echo "Crops grown in " . htmlspecialchars($district) . " during the " . htmlspecialchars($season) . " season are: ";

                                    // Encode inputs for the Python script
                                    $JsonState = escapeshellarg($state);
                                    $JsonDistrict = escapeshellarg($district);
                                    $JsonSeason = escapeshellarg($season);

                                    // Execute the Python script and capture output
                                    $command = "python ML/crop_prediction/ZDecision_Tree_Model_Call.py $JsonState $JsonDistrict $JsonSeason 2>&1";
                                    $output = shell_exec($command);

                                    // Display the output
                                    if ($output === null) {
                                        echo "<p>Error: No output received from the prediction script.</p>";
                                    } else {
                                        // Split the output into lines and display each crop
                                        $outputLines = explode("\n", trim($output));
                                        echo "<ul>";
